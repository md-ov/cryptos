...
class NumberHelperTest extends FunSuite {
    ...
    test("getMax") {
        val seq = Seq(1, 2, 5, 4, 3, 2.5, 4, 6, 7)
        val maxes: Seq[Double] = seq.getMax

        assert(maxes == Seq(1.0, 2.0, 5.0, 5.0, 5.0, 5.0, 5.0, 6.0, 7.0))
        assert(maxes.length == seq.length)
    }

    test("getMin") {
        val seq = Seq(7, 6, 5, 4, 5.5, 6.5, 3.5, 3, 2)
        val mins: Seq[Double] = seq.getMin

        assert(mins == Seq(7.0, 6.0, 5.0, 4.0, 4.0, 4.0, 3.5, 3, 2))
        assert(mins.length == seq.length)
    }
}
