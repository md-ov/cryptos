...
class ExpansionSegmentsTransformerForSizeModel(spark: SparkSession, transformedDataSchema: StructType) extends Transformer {
    
    override def transform(ds: Dataset[_]): DataFrame = {
        import spark.implicits._
        Expansion.expansion(spark, ds.as[Seq[BeforeSplit]]).withColumnRenamed("targetsize", label)
    }
    
    override def copy(extra: ParamMap): ExpansionSegmentsTransformerForSizeModel = this
